...
$stmt = $conn->prepare("
    SELECT 
        ...
        GROUP_CONCAT(room_images.image_url) AS roomImage
    FROM rooms r
...
if ($stmt->execute()) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // Convert room images from CSV string to array
        $row['roomImage'] = $row['roomImage'] ? explode(',', $row['roomImage']) : [];
        
        // Add the processed row to the rooms array
        $rooms[] = $row;
    }
